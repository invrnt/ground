...
Stage 4 integration repair grants from the Orchestrator: sourcing may edit `packages/server/src/modules/site/site-service.ts` solely to make history reads use SHARE locks, and `packages/server/src/modules/project/repository.ts` solely for explicit run-then-project SHARE lock order. Purchases may edit its owned service to make list reads use that same SHARE order. These repairs avoid upgrading snapshot read locks during report/evidence enrichment; mutation paths retain UPDATE locks.
...
AI Gateway selection grant from the Orchestrator: ai-gateway owns the neutral report-provider adapter and focused tests, interpretation imports, composition/configuration, operations status/export/redaction and current provider documentation. Shared edits are limited to the environment example, README, current contract/ownership records and manifests only if required. Previous owners are inactive for these paths; PRD, previous handoffs and recorded release evidence remain unchanged.

Easy-run grant from the Orchestrator: easy-run owns lifecycle/setup scripts and focused verification, package scripts, Compose/Dockerfile and current startup documentation. API/worker entrypoint changes are restricted to shutdown. Additional grants allow only active-SSE shutdown in `modules/live/index.ts` and correction of the confirmed extra-field error in `packages/contracts/src/events.ts` liveStateSchema. Previous owners are inactive; existing .env, PRD and previous handoffs remain unchanged.

...
import { projectModule,PgProjectRepository } from '../../packages/server/src/modules/project';
...
try { await configureDemo(tx,manifest,Object.fromEntries(manifest.members.map(m=>[m.username,password]))); const run=await seedDemo(tx,manifest); assert.equal(await seedDemo(tx,manifest),run); await configureDemo(tx,manifest,{});
 ...
 const member=await fixture.pool.query<{id:string}>("SELECT id FROM members WHERE username='ana'"); const actorId=member.rows[0]?.id; assert.ok(actorId); const snapshot=await new PgProjectRepository(tx).snapshot({actor_id:actorId,project_id:projectId,run_id:run,roles:['supervisor'],permissions:['costs'],trusted_time:new Date().toISOString()}); assert.equal(snapshot.stock.find(item=>item.fields['code']==='POR-GRIS-60')?.fields['quantity'],'8');

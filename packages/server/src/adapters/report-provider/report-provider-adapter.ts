...
import type { ReportProviderConfig } from './configuration';
...
export class ReportProviderAdapter {
  constructor(private readonly config:ReportProviderConfig,private readonly files:PrivateFileStore,private readonly fetcher:typeof fetch=fetch) {
    if(!config.api_key||!config.transcription_model||!config.interpretation_model)throw new GroundError('NOT_READY','Provider key and separate compatible model names are required.');
  }
  ...
  private metadata(data:z.infer<typeof responseSchema>,requested:string,requestId:string|null,start:number):ProviderMetadata {
    return {provider:this.config.provider,upstream_provider:data.provider??null,model:data.model??requested,request_id:data.id??requestId,duration_ms:Date.now()-start,usage:data.usage??null,cost:this.config.provider==='openrouter'?data.usage?.cost??null:null,prompt_version:PROMPT_VERSION,schema_version:SCHEMA_VERSION};
  }
  async transcribeAudio(bytes:Uint8Array):Promise<{text:string;derivative:Uint8Array;metadata:ProviderMetadata}> {
    ...
    try {
      const audio=Buffer.from(derivative).toString('base64');

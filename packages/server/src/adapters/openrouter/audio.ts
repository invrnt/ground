...
export async function wavAudio(bytes:Uint8Array, binary='ffmpeg'):Promise<Uint8Array> {
  if(bytes.byteLength>10*1024*1024)throw new GroundError('VALIDATION_ERROR','El audio supera 10 MB.');
  const pcm=await new Promise<Buffer>((resolve,reject)=> {
    ...
    const timer=setTimeout(()=>{process.kill('SIGKILL');finish(new GroundError('PROVIDER_UNAVAILABLE','La conversión de audio tardó demasiado.',true));},15000);
    process.on('error',()=>finish(new GroundError('NOT_READY','Audio conversion requires ffmpeg.')));
    process.stdout.on('data',(chunk:Buffer)=> {size+=chunk.length;if(size>61*32000){process.kill('SIGKILL');finish(new GroundError('VALIDATION_ERROR','El audio debe durar como máximo 60 segundos.'));}else chunks.push(chunk);});
    process.stderr.resume();
    process.stdin.on('error',()=>finish(new GroundError('VALIDATION_ERROR','No pude leer el audio.')));
    process.on('close',code=>finish(code===0?undefined:new GroundError('VALIDATION_ERROR','No pude leer el audio. Envía una grabación legible.')));
    process.stdin.end(bytes);
  });
  if(pcm.length===0||pcm.length>60*32000)throw new GroundError('VALIDATION_ERROR','El audio debe durar entre 1 y 60 segundos.');

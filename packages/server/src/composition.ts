...
export function createRuntime() { const pool=createPool(); const transactions=new PgTransactions(pool); const queue=new PgJobQueue(transactions); const files=new LocalPrivateFiles(process.env['PRIVATE_STORAGE_PATH']??'/tmp/ground-private'); const sessions=new Sessions(pool,process.env['PUBLIC_BASE_URL']??'http://localhost:3000'); return {pool,transactions,queue,files,sessions}; }

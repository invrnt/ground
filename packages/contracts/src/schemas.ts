...
export const projectSnapshotSchema=z.object({project_id:idSchema,run_id:idSchema,scenario_version:z.string(),scenario_date:dateSchema,project_version:z.number().int(),event_cursor:z.number().int(),members:z.array(entitySchema),locations:z.array(entitySchema),work:z.array(entitySchema),stock:z.array(entitySchema),issues:z.array(entitySchema),tasks:z.array(entitySchema),needs:z.array(procurementNeedSchema),proposals:z.array(requestProposalSchema),requests:z.array(outboundRequestSchema),decisions:z.array(approvalDecisionSchema),remote_links:z.array(externalObjectLinkSchema)}).strict();
